...
class DRLThresholdLearningEnv(Env):
    def __init__(self, channel_contents,
        ...
        :param channel_contents: channel state information, including channel matrix, assumed to be imported from .mat file as a dictionary type data
                                 python dictionary type data, with keywords 'H'. Size is (N_r,N_t,realization,M,K)
        ...
        self.subcarrier_total_number = 16
        self.subcarrier_pilot_number = 16
        self.num_of_pilots = 0 
        self.block_size = 0
    ...
    def _choose_next_state(self.state):        
        ...
        if transmission_mode == 0: #When transmission mode is channel estimation
            ...
            #Update state
            self.state = temp
            #Update block size and ratio of pilots
            self.block_size = 0
            self.num_of_pilots = self.num_of_pilots + self.subcarrier_pilot_number
            ...
            #Update block size and ratio of pilots
            self.block_size = self.num_of_pilots + self.subcarrier_total_number
            self.num_of_pilots = 0
    ...
    def _get_reward(self, action):
        ...
        if relay_index == 0:
            R = SE_computation(Tx_codebook_index,Rx_codebook_index,channel_Tx_Rx[:,:,0,current_step,:],SNR,subcarrier_total_number)
        if relay_index == 1:
            R_1st_hop = SE_computation(Tx_codebook_index,relay_codebook_index,channel_Tx_relay[:,:,0,current_step,:],SNR,subcarrier_total_number)
            R_2nd_hop = SE_computation(relay_codebook_index,Rx_codebook_index,channel_relay_Rx[:,:,0,current_step,:],SNR,subcarrier_total_number)
    ...
    def SE_computation(i_index,j_index,channel,SNR,subcarrier_total_number):
        # Assuming transmit/receive codebook size is number of transmit/receive antennas and equal beamwidth
        R = 0
        for subcarrier in range(subcarrier_total_number):
            ...
            mmse = 1/(num_of_pilots*SNR)
            effective_SNR = SNR*(1-mmse)/(1+SNR*mmse)
            _, R_temp = np.linalg.slogdet(np.identity(N_S)+effective_SNR/N_S*np.linalg.multi_dot([pinv(R_n),conjT(A_r),channel,A_t,conjT(A_t),conjT(channel[:,:,subcarrier]),A_r]))    
            R = R + R_temp
        R = R / subcarrier_total_number
        return np.log2(2.718281)*R

    def beam_sweeping(self.current_step,channel,N_t,N_r,SNR):
        i_star = 0
        j_star = 0
        maximum_beam_measurement = 0
        for i_dummy in range(N_t):
            for j_dummy in range(N_r):
                beam_measurement = self.SE_computation(i_dummy,j_dummy,channel,SNR,subcarrier_total_number)
